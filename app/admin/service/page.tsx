...
const Page = () => {
  ...
  //Add new service
  // Add a state for creating a new service
  const [newService, setNewService] = useState({
    name: "",
    actionDescription: "",
    floorPrice: 0,
    ceilPrice: 0,
    serviceType: "Main Service", // Default type
  });
  const {
    isOpen: isCreateOpen,
    onOpen: onCreateOpen,
  ...
  // Handle input changes for creating a new service
  ...
  const handleAddService = () => {
    try {
      axiosClient
        .post("config-services", newService)
        .then(() => {
          fetchService();
          ...
          //   onCreateOpenChange(false); // Đúng cách để đóng modal
        })
        .catch(() => {
  ...
  return (
    <div className="flex flex-col justify-start w-full mx-10 gap-5 my-3">
      ...
      <div className="flex justify-between items-center">
        <Button color="primary" onClick={onCreateOpen}>
          Tạo mới
        </Button>
      </div>
      <Table
      ...
      <Modal isOpen={isCreateOpen} onOpenChange={onCreateOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader>Tạo mới dịch vụ</ModalHeader>
              <ModalBody>
                <div>
                  <h2>Tên dịch vụ</h2>
                  <Input
                  ...
                  <Input
                    ...
                    value={newService.actionDescription}
                    onChange={handleNewInputChange}
                  />
                  <h2>Giá thấp nhất</h2>
                  <Input
                    ...
                    variant="bordered"
                    name="floorPrice"
                    value={newService.floorPrice.toString()}
                    onChange={handleNewInputChange}
                  />
                  <h2>Giá cao nhất</h2>
                  <Input
                    ...
                    variant="bordered"
                    name="ceilPrice"
                    value={newService.ceilPrice.toString()}
                    onChange={handleNewInputChange}
                  />
                  <h2>Loại dịch vụ</h2>
                  <Input
